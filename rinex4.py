            ...
            while True:
                ...
                if re.match(r"^>\s\d{4}\s\d{2}\s\d{2}", line):
                    ...
                    for _ in range(satellites_count):
                        ...
                        prn = int(svprn[1:])
                        c1 = float(line[3:17].strip()) if line[3:17].strip() else None
                        d1 = float(line[19:33].strip()) if line[19:33].strip() else None
                        s1 = float(line[35:49].strip()) if line[35:49].strip() else None
